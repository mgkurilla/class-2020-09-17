...
en_fall_2020 <- read_excel("raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx", skip = 3) %>%
  filter(UGrad >= 10) %>%
  select("Course ID", "Course Name", "Course Department", "Instructor Full Name", "UGrad") %>%
  arrange(desc(UGrad)) %>%
  drop_na()
en_fall_2020
```
Answer: Principles of Economics (more commonly known as Ec10) is the course with
the highest enrollment. Data (more commonly known as Gov50) has an enrollment
of 110 undergraduates. 
...
en_fall_2020 %>%
  slice(1:10) %>%
  ggplot(en_fall_2020, aes(x = Course ID, y = UGrad)) + 
  geom_col()
```
